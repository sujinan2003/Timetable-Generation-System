...
def load_courses_curriculum(client):
    ...
    try:
        ...
        for sheet in openCourseSheets:
            ...
            if not sheet.title.endswith('_Y1'):
...
'''
def load_room_availability(client):
    room_availability = {}
    try:
        room_file = client.open('Room')
        sheets = room_file.worksheets()
        ...
        for sheet in sheets:
            ...
            if not data or len(data) < 3:
                print(f"Insufficient data in sheet {room_id}.")
                continue
            ...
            if availability:
                room_availability[room_id] = {'availability': availability}
        
    except Exception as e:
        print(f"Failed to load room availability: {e}")
...
class TimeTable:
    ...
    '''
    #ตรวจสอบว่าห้องว่างไหมในวันและคาบที่ต้องการ
    def check_room_availability(self, room_id, day, period):
        if room_id not in self.room_availability:
            return False  # ถ้าไม่มีข้อมูลห้อง ให้ถือว่าห้องไม่ว่าง (เพื่อความปลอดภัย)
        availability = self.room_availability[room_id]['availability']
        return availability.get(period, {}).get(day, 0) == 1  # ถ้าเท่ากับ 1 แสดงว่าห้องว่าง
    '''
    ...
    def check_schedule_conflict(self, day, start_period, end_period, room, section):
        for entry in self.schedule:
            # ตรวจสอบว่ารายการอยู่ในวันเดียวกันและใช้ห้องเรียนเดียวกัน
            if entry['วันเรียน'] == day and entry['เซคเรียน'] == section:  # เช็คเซคเรียน
                # ตรวจสอบการชนกันของเวลา
...
def run():
    ...
    #room_availability = load_room_availability(client)
    student_availability = check_timetable_student(client)
    
    '''
    print("Student availability data:")
    for section, data in student_availability.items():
    ...
    write_timetable_to_sheet(best_timetable, 'Gen_Y1')
