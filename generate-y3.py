...
def load_courses_curriculum(client):
    ...
    try:
        ...
        for sheet in openCourseSheets:
            # เพิ่มเงื่อนไขเพื่อเลือกเฉพาะชีทปี 3
            if not sheet.title.endswith('_Y3'):
...
def check_timetable_student_generateFile(client):
    ...
    try:
        ...
        for sheet in sheets:
            if not sheet.title.endswith('Gen_Y1') and not sheet.title.endswith('Gen_Y2'):
                continue  
...
def run():
    ...
    write_timetable_to_sheet(best_timetable, 'Gen_Y3')
